...
products.forEach((product, position) => console.log(`Name: ${product.name} \t Price: ${product.price}`))

const productsInStock = products.filter((product, _) => {
  if(product.inStock){
    return product;
  }
});

const electicProducts = products.filter((product, _) => {
  if(product.tags.includes('electronic') || product.category == 'Electronics'){
    return product;
  }
});

const findBlender = products.find((product, _) => {
  if(product.name.toLowerCase() == 'blender'){
    return product;
  }
});

console.log(productsInStock);
console.log(electicProducts);
console.log(findBlender);